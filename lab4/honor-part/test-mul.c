/*
 * test-mul.c - Checks the correctness and performance of all of the
 *     student's multiply functions and records the results for their
...
/* The description string for the multiply_submit() function that the
   student submits for credit */
#define SUBMIT_DESCRIPTION "multiply submission"

/* External function defined in mul.c */
...
/* The correctness and performance for the submitted multiply function */
struct results
{
    int funcid;
    int correct;
...
/*
 * eval_perf - Evaluate the performance of the registered multiply functions
 */
void eval_perf(unsigned int s, unsigned int E, unsigned int b)
{
    ...
    /* Evaluate the performance of each registered multiply function */
    ...
    for (i = 0; i < func_counter; i++)
    {
        ...
        /* Save the correctness of the multiply submission */
        if (results.funcid == i)
        {
            results.correct = 1;
        }
        ...
        /* Filtered trace for each multiply function goes in a separate file */
        sprintf(filename, "trace.f%d", i);
        part_trace_fp = fopen(filename, "w");
        assert(part_trace_fp);

        /* Locate trace corresponding to the multiply function */
        flag = 0;
        ...
        /* If it is multiply_submit(), record number of misses */
        if (results.funcid == i)
        {
